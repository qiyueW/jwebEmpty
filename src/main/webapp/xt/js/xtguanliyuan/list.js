...
function dellRow() {
	...
	easyuiAjax("/xt/xtguanliyuan1/remove.jw", {
...
function update01() {// 审核
	...
	easyuiAjax("/xt/xtguanliyuan1/update/examine.jw", {
		ids : easyuiGetRowsID(rows, 'xt_guanliyuan_zj')
	}, "请确认审核操作", function() {
		easyuiGridReload('dg')
	});
}
function update10() {// 反审核
	...
	easyuiAjax("/xt/xtguanliyuan1/update/unexamine.jw", {
		ids : easyuiGetRowsID(rows, 'xt_guanliyuan_zj')
	}, "请确认反审核操作", function() {
		easyuiGridReload('dg')
	});
...
function update04() {// 作废
	...
	easyuiAjax("/xt/xtguanliyuan1/update/void.jw", {
		ids : easyuiGetRowsID(rows, 'xt_guanliyuan_zj')
	}, "请确认作废操作", function() {
		easyuiGridReload('dg')
	});
...
function update40() {// 反作废
	...
	easyuiAjax("/xt/xtguanliyuan1/update/unvoid.jw", {
		ids : easyuiGetRowsID(rows, 'xt_guanliyuan_zj')
	}, "请确认反作废操作", function() {
		easyuiGridReload('dg')
	});
}
function f_resetpassword() {// 重置总管密码
	var rows = $('#dg').datagrid('getSelections');
	if (!rows[0]) {
		$.messager.alert('提示', '请选择行');
		return;
	...
	$.messager.prompt('重置密码', '请输入重置后的密码', function(r) {
		if (r) {
			easyuiAjax("/xt/xtguanliyuan1/update/password2.jw", {
				newpassword : r,
				id : rows[0]['xt_guanliyuan_zj']
