...
final public class XtRYQuanxianService {
	...
	private static final String ADMIN_M_RY_BM// 检出指定隔离标识部门的人员
			= "SELECT R.*,RQ.xt_ryquanxian_zj,RQ.xt_guanliyuan_mc " + "FROM " + "(" + "SELECT * FROM RY WHERE RY.ry_gelibiaoshi='?1' AND RY.ry_bm_zj IN(?2) ?condition "// 从人员表中，取出相同隔离标识的，某部门的人员
					+ ") R " + "LEFT JOIN " + "(SELECT XtRYQuanxian.xt_ryquanxian_zj,XtRYQuanxian.xt_ry_zj,XtGuanliyuan.xt_guanliyuan_mc  FROM XtRYQuanxian "// 从人员权限表中，检查 定制人的信息
					+ "	 LEFT JOIN XtGuanliyuan ON XtRYQuanxian.xt_ryquanxian_zhidanren=XtGuanliyuan.xt_guanliyuan_zj" + ")RQ " + "ON R.ry_zj=RQ.xt_ry_zj;";
	private static final String ADMIN_M_RY// 检出指定隔离标识的人员
			= "SELECT R.*,RQ.xt_ryquanxian_zj,RQ.xt_guanliyuan_mc " + "FROM " + "(" + "SELECT * FROM RY WHERE RY.ry_gelibiaoshi='?1' ?condition "// 从人员表中，取出相同隔离标识的，某部门的人员
					+ ") R " + "LEFT JOIN " + "(SELECT XtRYQuanxian.xt_ryquanxian_zj, XtRYQuanxian.xt_ry_zj,XtGuanliyuan.xt_guanliyuan_mc  FROM XtRYQuanxian "// 从人员权限表中，检查 定制人的信息
					+ "	 LEFT JOIN XtGuanliyuan ON XtRYQuanxian.xt_ryquanxian_zhidanren=XtGuanliyuan.xt_guanliyuan_zj" + ")RQ " + "ON R.ry_zj=RQ.xt_ry_zj;";
	// ---------------------------------------查询---------------------------------------
	...
	public static List<ViewRYQuanxian> select(final String gelibiaoshi, final String bm_zj, String condition) {
		if (null == bm_zj || bm_zj.isEmpty()) {
			return DBO.service.ADUS.executeQueryVast(ViewRYQuanxian.class, ADMIN_M_RY.replace("?1", gelibiaoshi).replace("?condition", condition));
		} else {
			return DBO.service.ADUS.executeQueryVast(ViewRYQuanxian.class, ADMIN_M_RY_BM.replace("?1", gelibiaoshi).replace("?2", Tool.replaceDToDDD(bm_zj)).replace("?condition", condition));
		}
	}
...
//---------------------------------------ADU--------------------------------------
	...
	public static MsgVO addOne(XtRYQuanxian obj) {
		...
		if (i < 1) {
			return MsgVO.setError("权限绑定失败");
		}
		return MsgVO.setOK("权限绑定成功");
	}
	...
	public static MsgVO dellVast(String ids) {
		int i = DBO.service.D.dellByCondition(XtRYQuanxian.class, "WHERE xt_ryquanxian_zj IN(" + Tool.replaceDToDDD(ids) + ")");
		if (i < 1) {
			return MsgVO.setError("没有可操作的单据");
		}
		return MsgVO.setOK("移除成功");
	}
	public static MsgVO dellVast(String ids,String zhidanren_zj) {
		int i = DBO.service.D.dellByCondition(XtRYQuanxian.class, "WHERE xt_ryquanxian_zj IN(" + Tool.replaceDToDDD(ids) + ") AND xt_ryquanxian_zhidanren='"+zhidanren_zj+"'");
		if (i < 1) {
			return MsgVO.setError("没有可操作的单据");
		}
		return MsgVO.setOK("移除成功");
	...
	public static MsgVO update(XtRYQuanxian obj) {
		int i = DBO.service.U.updateSome_alloy(obj, "xt_juese_zj,xt_quanxian");
		if (i < 1) {
			return MsgVO.setError("权限绑定失败");
		}
		return MsgVO.setOK("权限绑定成功");
