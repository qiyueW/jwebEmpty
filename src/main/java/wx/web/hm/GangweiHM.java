...
//===================查询操作============================= 
   ...
   //@system.web.power.ann.SQ("BS")
    ...
    // jw.printOne(Tool.entityToJSON(DBO.service.S.select(LoufangFL.class, "order by loufangfl_px ASC"))); 如果有排序的话，将loufangfl_px字段替换为你的项目中的字段
        jw.printOne(Tool.entityToJSON(GangweiService.select()).replace("\n", "/n"));
    }
   //@system.web.power.ann.SQ("BS")
    @M("/select/json2")
    public static void select2(JWeb jw) {
        jw.printOne(EasyuiService.formatTree(GangweiService.select(), "gangwei_zj", "gangwei_fzj", "gangwei_mc").replace("\n", "/n"));
    }
...
//---------------------------------------单据状态管理---------------------------------------
    ...
    public void examine() {
        jw.printOne(GangweiService.updateStyle_examine(jw.getString("ids")));
    }

    @M("/update/unexamine")//反审核
    public void unexamine() {
        jw.printOne(GangweiService.updateStyle_unExamine(jw.getString("ids")));
    }

    @M("/update/void")//作废
    public void tovoid() {
        jw.printOne(GangweiService.updateStyle_void(jw.getString("ids")));
    }

    @M("/update/unvoid")//反作废
    public void untovoid() {
        jw.printOne(GangweiService.updateStyle_unVoid(jw.getString("ids")));
    }
