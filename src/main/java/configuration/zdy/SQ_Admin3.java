 ...
 * @author wangchunzi
 */
public class SQ_Admin3 implements IZDY {
    ...
    public boolean denyByTrue(JWeb jw, String urlCode) {
        ...
        if (urlCode.isEmpty()) {
//            System.out.println("检验成功，辅管已经登陆。");
            return false;
        }
        @SuppressWarnings("rawtypes")
		PISD pobj = (PISD) admin;
        XtGuanliyuan xtadmin = (XtGuanliyuan) pobj.obj;
        if (xtadmin.getXt_guanliyuan_jibie() != 3) {//不是辅管的，检验不合法。
            jw.printOne(MsgVO.setError("不是辅管的，检验不合法"));
            return true;
        }
        ...
        if (Login.isNoThisPower(pobj, urlCode)) {
            jw.printOne(MyPowerCheck.SQ_ERROR3);
            return true;
        }
        return false;
    }
